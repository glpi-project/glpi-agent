...
sub _getESETInfo {
    ...
    # Check if com.eset.protection is registered / running in launchctl
    my $daemon_status = getFirstLine(
        file    => $params{launchctl_list}, # For unit tests
        command => 'launchctl list com.eset.protection',
        %params
    );
    my $launchctl_running = (defined $daemon_status && $daemon_status !~ /Could not find/) ? 1 : 0;
    ...
    $antivirus->{ENABLED} = ($launchctl_running && $startd_running && $rtp_enabled) ? 1 : 0;
